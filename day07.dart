...
int do2(Map<String, Tower> towers) {
  ...
  return findUnbalanced(bottom, towers, WeightFactory(towers))!;
}

int? findUnbalanced(String tname, Map<String, Tower> towers, WeightFactory weights) {
  final tower = towers[tname]!;
  if (tower.children.isEmpty) return null;
  if (tower.children.length == 1) return null;
  final ws = tower.children.map((c) => weights.getWeight(c)).toList();
  ...
  final unbalanced = tower.children.where((c) => weights.getWeight(c) == single).single;
  final childUnbalanced = findUnbalanced(unbalanced, towers, weights);
  if (childUnbalanced != null) return childUnbalanced;
  return towers[unbalanced]!.weight - (weights.getWeight(unbalanced) - goalWeight);
}
